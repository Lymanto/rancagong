...
export default function ModalEdit({
  ...
  aparatur: AparaturType;
  onClickCancel: ButtonHTMLAttributes<HTMLButtonElement>['onClick'];
}) {
  const [name, setName] = useState(aparatur.name);
  const [position, setPosition] = useState(aparatur.position);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    formData.append('id', aparatur.id.toString());
    formData.append('imageId', aparatur.imageId.toString());
    if (name === '' || position === '') {
      setError('name, position tidak boleh kosong');
      return;
    }

    setLoading(true);
    await axios
      .put('/api/aparatur', formData)
      .then((res) => {
        setLoading(false);
      })
      .catch((err) => {
